...
class PasswordCheckAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not password:
            return Response({"detail": "Пароль не передан"}, status=status.HTTP_400_BAD_REQUEST)
        user = request.user
        if user.check_password(password):
            return Response({"password_valid": True})
        else:
            return Response({"password_valid": False}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def perform_create(self, serializer):
        ...
        if parent_id:
            try:
                ...
                raise serializers.ValidationError({"parent": "Родительский комментарий не найден."})
...
# Остальные классы (PostRepostAPIView, PostLikeAPIView и т.п.) без изменений
